...
void ShiftMETforTES::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    for(pat::TauCollection::const_iterator inputTau = tauHandle->begin(); inputTau != tauHandle->end(); ++inputTau, ++itau)
    {
      ...
      bool existUp = userdatahelpers::hasUserInt(l,"TauUpExists"); // simply to check if the userfloat exists, i.e. if it is a tau
      int  hasUp   = ( existUp ? userdatahelpers::getUserInt(l,"TauUpExists") : false) ;   // actual check of the value of the userfloat
      ...
      bool existDown = userdatahelpers::hasUserInt(l,"TauDownExists"); // simply to check if the userfloat exists, i.e. if it is a tau
      int  hasDown   = ( existDown ? userdatahelpers::getUserInt(l,"TauDownExists") : false) ; // actual check of the value of the userfloat
